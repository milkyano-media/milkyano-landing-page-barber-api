...
export default class ParameterService {
     ...
     */
    async listParameterCategories() {
        const parameterCategories = await this.prisma.parameter.findMany({
            select: {
                category: true,
            },
            distinct: ["category"],
        ...
        const result = parameterCategories.map((category) => category.category.toLowerCase());
